...
export async function createPart(
    ...
    try {
        ...
        const result = await sql.begin(async (transaction) => {
            ...
            // Insert the part version record with ALL fields from the form
            await transaction`
            INSERT INTO "PartVersion" (
                ...
                ${input.technicalSpecifications ? sql.json(input.technicalSpecifications) : null},
                ${input.properties ? sql.json(input.properties) : null},
                ${input.electricalProperties ? sql.json(input.electricalProperties) : null},
                ${input.mechanicalProperties ? sql.json(input.mechanicalProperties) : null},
                ${input.thermalProperties ? sql.json(input.thermalProperties) : null},
                ${input.materialComposition ? sql.json(input.materialComposition) : null},
                ${input.environmentalData ? sql.json(input.environmentalData) : null},
                ${input.revisionNotes || null},
                ${input.dimensions ? sql.json(input.dimensions) : null},
